"""
V2 Review Routes with Pagination

Enhanced review endpoints with:
...
@review_router.get(
    "/",
    response_model=PaginatedResponse[ReviewFull],
    status_code=status.HTTP_200_OK,
    summary="Get all reviews (paginated)",
    description="Retrieve all reviews with pagination, reviewer and book information.",
...
async def get_all_reviews(
        ...
        max_rating: Optional[int] = Query(default=None, ge=1, le=5, description="Maximum rating filter")
) -> PaginatedResponse[ReviewFull]:
...
@review_router.get(
    "/my-reviews",
    response_model=PaginatedResponse[ReviewWithBook],
    status_code=status.HTTP_200_OK,
    summary="Get my reviews (paginated)",
    description="Get all reviews written by the current user with pagination.",
...
async def get_my_reviews(
        ...
        page: int = Query(default=1, ge=1, description="Page number"),
        page_size: int = Query(default=10, ge=1, le=100, description="Items per page")
) -> PaginatedResponse[ReviewWithBook]:
    """Get current user's reviews with pagination."""
    reviews, total = await review_service.get_my_reviews_paginated(
...
@review_router.get(
    "/book/{book_uuid}",
    response_model=PaginatedResponse[ReviewWithReviewer],
    status_code=status.HTTP_200_OK,
    summary="Get reviews for a book (paginated)",
    description="Get all reviews for a specific book with pagination.",
...
async def get_reviews_by_book(
        ...
        page: int = Query(default=1, ge=1, description="Page number"),
        page_size: int = Query(default=10, ge=1, le=100, description="Items per page")
) -> PaginatedResponse[ReviewWithReviewer]:
...
@review_router.get(
    ...
    response_model=PaginatedResponse[ReviewWithBook],
    status_code=status.HTTP_200_OK,
    summary="Get reviews by user (paginated)",
    description="Get all reviews written by a specific user with pagination.",
    responses=PUBLIC_RESPONSES
)
async def get_reviews_by_user(
    ...
    reviews, total = await review_service.get_reviews_by_user_paginated(
        user_uuid=user_uuid,
        session=session,
        page=page,
        page_size=page_size
